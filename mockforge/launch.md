...
- [ ] Corregir el serving de assets locales/fallback que hoy apuntan a `/api/uploads/:file` sin route handler real.
- [ ] Validar que una imagen subida se pueda:
  - [ ] previsualizar en `/upload`
  - [ ] consumir en `/results`
  - [ ] reutilizar en variaciones, upscale y video
- [ ] Confirmar que el fallback local funciona aunque falle Supabase Storage o Bunny.
- [ ] Añadir test que cubra el path real devuelto por `saveInputUpload`.
...
- [ ] Mover la deducción de créditos para que no ocurra antes de validar configuración mínima del provider.
- [ ] Asegurar que no se pierdan créditos cuando la generación falla.
- [ ] Definir comportamiento explícito para batch parcial:
  - [ ] cobrar solo éxitos
  - [ ] o cobrar upfront y reembolsar fallidos
...
- [ ] Corregir el error actual de React Hooks en `onboarding-tour.tsx`.
- [ ] Resolver o aceptar explícitamente los warnings de hooks restantes.
- [ ] Hacer que `npm run build` sea reproducible en CI/producción.
...
- [ ] Configurar `playwright.config.ts` con `webServer` o equivalente.
...
- [ ] Cambiar `/api/admin/costs` a modo fail-closed si falta `ADMIN_SECRET`.
- [ ] Verificar que `/api/admin/cleanup` requiere secreto en todos los entornos públicos.
- [ ] Revisar si `/api/debug/upload` y `/debug/upload` deben quedar accesibles en producción.
- [ ] Si no deben existir públicamente:
  - [ ] deshabilitarlos en producción
  - [ ] o protegerlos por secreto fuerte
- [ ] Revisar exposición de `/api/provider/health` y decidir cuánta información debe devolver públicamente.
...
- [ ] Todos los bloqueadores de la sección 2 están completos
